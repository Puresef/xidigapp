...
- **System timeout demotion (decided 12 Aug 2026):** Maal auto-demotes back to Warshad through the system timeout path. Demotion is system-driven only — never user-initiated — is publicly logged in the Governance Log, and preserves history in full: the venture's work history, ledger, contribution record, and prior decisions all survive; only the current stage/status changes. *(The timeout path is not yet implemented; it ships in Maal F2.)* Below the Maal stage, a stalled Lab still receives dormancy *encouragement* prompts (see Dormant below) — direction there is encouragement, not punishment.
