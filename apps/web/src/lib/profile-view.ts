...
export async function getPublicProfileView(handle: string): Promise<ProfileView | null> {
  ...
  // A quarantined test account is not a real member: no public page, no OG
  // card, nothing crawlable. Members get the test-account notice instead
  // (getMemberProfileView).
  const flags = await loadAccountFlags(admin, [row.user_id]);
  if (isTestAccount(flags, row.user_id)) return null;
