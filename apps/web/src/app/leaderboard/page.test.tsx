...
function member(id: string, score: number, opts: { isTest?: boolean } = {}) {
  db.users.push({ id, status: 'active', is_ai: false, is_test: opts.isTest ?? false });
...
describe('leaderboard — quarantined test accounts never rank', () => {
  ...
  it('with no test accounts, every positive score ranks as before', async () => {
    member('a', 30);
    member('b', 20);

    expect(rankedHandles(await render())).toEqual(['h_a', 'h_b']);
  });
});
