 ...
 *     included), no verification chip, no counts or reputation, no
 *     follow / message / share controls, no hosted events;
 *   * the metadata is noindex/nofollow for every viewer.
 *
 * The projection chain runs as shipped (lib/profile-view → lib/account-flags)
 * over fake clients; only request plumbing and the interactive controls are
 * stubbed — the controls as sentinels, so the real-profile control case can
 * prove the absences are not vacuous.
 */
...
const FOUNDING_BADGE: Row = {
  ...
  context: null,
  badge_definitions: { slug: 'founding-member', name: 'Founding Member', description: null },
};
...
describe('/u/[handle] for a real account — the control case', () => {
  ...
  it('renders the public profile for a signed-out visitor', async () => {
    h.admin = adminFor(false);
