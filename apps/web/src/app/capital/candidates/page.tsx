...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return frontMetadata({
    ...
    path: '/capital/candidates',
  });
}

/**
 * The Phase-5 candidate board (§17). Unchanged in substance — it moved here
 * from `/capital` when `/capital` became the Maal index (plan D1), keeping its
 * status filter tabs, its cards, its teaching empty state and its loading
 * shell. Nothing about candidates was deleted or redesigned; only its address
 * changed, and the Maal index links to it.
 *
 * Lists the candidates the viewer can read (RLS scopes the fetch — draft /
 * reviewers-only rows they lack access to simply don't appear). Status filter
 * tabs are shareable `?status=` links (no JS).
 */
...
export default async function CapitalCandidatesPage({
  searchParams,
}: {
  ...
  return (
    <main className="xidig-section">
      <BackLink href="/capital" labelKey="maal.backToIndex" />
      <div className="xidig-card__header">
        <h1 className="xidig-auth__title">{t('capital.candidatesTitle')}</h1>
      </div>
      ...
      <div className="xidig-tabs">
        ...
        {STATUSES.map((s) => (
          <Link
            key={s}
            className="xidig-tabs__tab"
            href={`/capital/candidates?status=${s}`}
