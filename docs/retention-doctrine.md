...
| `candidate_reviews.notes` (reviewer), `candidate_votes`, `interests` | M (review/governance/intent records); **reviewer notes with deleted-member data = restricted (ruled §1b.3)**       | notes member-readable; the vote is paused and no tally is shown (P1)                    | notes: ✗ → plan §3.6; votes: aligned; interests: Q2 excluded                |
...
| `verifications` (+ recording pointer), `verification_access_log` | M/L                                                       | kept; the recording expiry is DECLARED but NOT enforced (nothing writes `recording_expires_at`; no recording writer today) | ✗ latent (DPIA) — must be fixed before any recording is written; 1-year step missing |
