...
12. **Brand mark redraw** — **RESOLVED 17 Jul as a lossless optimization** (see `docs/brand-direction.md` §1): the five-shard replacement was rejected (the blue X stays); `icon.svg` was re-emitted by trace-simplification of its own paths — 19,052 B → 1,370 B, verified pixel-identical at all favicon sizes. No aesthetic change shipped, so the fidelity sign-off gate was not tripped; any *aesthetic* redesign remains Warya's later call. Separable-arm assemble-on-open motion would need a geometric rebuild (the original's paths aren't arm-separable) — parked.
