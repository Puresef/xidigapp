...
| **Reviewer notes** (`candidate_reviews.notes`)                   | A deleted reviewer's notes stay readable by every member who can read the candidate, with no reviewer-status clause (`20260707000000_phase5_capital.sql:205-212`), yet the doctrine classes them as restricted. They are §17-transparency-locked.                                                                                                                                                                                                                                                                                      | C3/C4    | **owner ruling** (§17 lock)                      |
...
- **Base.** No existing worktree holds the full migration line:
  - this branch lacks Packet B's `20260911001000` and naming's
    `20260912000000`;
  - naming lacks every deletion migration.

  R1b therefore starts from **one integration branch, shared with the Xidig
  Plus mechanics slice** (`docs/xidig-plus-doctrine.md` §4 on
  `claude/packet-b-support-taageer-xidig-plus`). It holds this line
  (`cbcc297` ⊃ `94dfc01`, `998f991`, `82daa4c`, `7162c1c`), Packet B
  (`42d10e3`) and naming (`384b840`). It needs `cbcc297` because R1b's
  Space-history step recreates the `20260911001100` policies, which exist
  only here. That is a merge, gated on the owner. Every new migration in
  either lane sorts after `20260912000000`; reserve numbers for both lanes,
  and sequence their `en.ts`/`so.ts` edits.
